...
public:
  ...
  bool         do_also_cells       = true; // test also cell-based agglomeration
  bool         do_also_points = true; // test also point-based agglomeration
...

{
  ...
  enter_subsection("R3MG");
  {
    ...
    add_parameter("Do also point agglomeration", do_also_points);
    add_parameter("Do also cell agglomeration", do_also_cells);
  }
  leave_subsection();
...
public:
  ...
  static constexpr unsigned int rtree_m_points = 4; // 2; // 4;
...
Poisson<dim>::Poisson(const ProblemParameters<dim> &problem_parameters)
  ...
  , output_info_filename(
      ...
      (parameters.do_also_points ? "_point_agglo" : "") +
      (parameters.do_also_cells ? "_cell_agglo" : "") +
...
Poisson<dim>::setup_multigrid()
{
  ...
  Utils::MGCoarseDirectMUMPS<TrilinosWrappers::SparseMatrix,
                             ...
                             SparseDirectMUMPS>
    mg_coarse;
  mg_coarse.initialize(*multigrid_matrices[min_level]); // TODO
  ...
  if (file.is_open())
    {
...

      {
        ...
        write_stat_line(file, "H_coarse/h_fine", out.str());
      }
...
Poisson<dim>::run()
{
  ...
  assemble_system();
  {
    ...
    if (file.is_open())
      {
        file << "\n" << std::string(80, '=') << std::endl;
        file << "Number of DoFs : " << original_dof_handler.n_dofs()
             << std::endl;
        file << std::string(80, '=') << std::endl;
      }
    file << std::string(80, '=') << std::endl;
